    ...
    function createFileBlock(fileName, content) { const block = document.createElement('div'); block.className = 'file-block'; block.innerHTML = '<h4>' + fileName + '</h4><pre class="bg-black p-2 rounded"><code>' + escapeHtml(content) + '</code></pre>'; return block; }
        ...
        for (const url of urls) {
            try {
                ...
                const jsFiles = links
                    ...
                    .filter(name => name && name.endsWith('.js') && !name.startsWith('.') && name !== 'gitauto.js');
    ...
    async function loadFiles() {
        ...
        try {
            ...
            if (isLocalHost) {
                ...
                if (Array.isArray(data)) {
                    files = data
                        .filter(item => item.type === 'file' && item.name.endsWith('.js') && item.name !== 'gitauto.js')
                        .map(item => item.name)
                        .sort();
                }
            }
        ...
        const visibleFiles = files.filter(fileName => fileName !== 'gitauto.js');
